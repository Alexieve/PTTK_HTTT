...
END;
/
GRANT EXECUTE ON USP_HSUNGTUYEN_GET_CHITIET_HSUNGTUYEN TO RL_NHANVIEN, RL_DOANHNGHIEP, RL_UNGVIEN;
...
CREATE OR REPLACE PROCEDURE USP_HSUNGTUYEN_GET_FOR_XAC_THUC_HS(P_RES OUT SYS_REFCURSOR)
AS
BEGIN
    OPEN P_RES FOR
        SELECT HS.MAUV, HS.MAHOPDONG, UV.HOTEN, HS.NGAYNOP
        FROM HSUNGTUYEN HS
        JOIN UNGVIEN UV ON HS.MAUV = UV.MAUV
        WHERE KETQUA = 0;
END;
/
GRANT EXECUTE ON USP_HSUNGTUYEN_GET_FOR_XAC_THUC_HS TO RL_NHANVIEN;



CREATE OR REPLACE PROCEDURE USP_HSUNGTUYEN_GET_FOR_KET_QUA_UNG_TUYEN(P_RES OUT SYS_REFCURSOR)
AS
BEGIN
    OPEN P_RES FOR
        SELECT DN.TENDN, HS.MAUV, HS.MAHOPDONG, CB.MOTA || ' - ' || VT.MOTA AS VITRI, HS.NGAYNOP, CASE
                WHEN KETQUA = 0 THEN N'Đang chờ xác thực'
                WHEN KETQUA = 1 THEN N'Cần hiệu chỉnh'
                WHEN KETQUA = 2 THEN N'Đang chờ duyệt'
                WHEN KETQUA = 3 THEN N'Không đủ điều kiện'
                WHEN KETQUA = 4 THEN N'Đang chờ DN phản hồi'
                WHEN KETQUA = 5 THEN N'Không đạt'
                WHEN KETQUA = 6 THEN N'Đạt'
                END AS KETQUA
        FROM HSUNGTUYEN HS
        JOIN HOPDONG HD ON HS.MAHOPDONG = HD.MAHOPDONG
        JOIN DOANHNGHIEP DN ON HD.MADN = DN.MADN
        JOIN CAPBAC CB ON HD.CAPBACTD = CB.MACB
        JOIN VITRI VT ON HD.VITRITD = VT.MAVT;
END;
/
GRANT EXECUTE ON USP_HSUNGTUYEN_GET_FOR_KET_QUA_UNG_TUYEN TO RL_UNGVIEN;


CREATE OR REPLACE PROCEDURE USP_HSUNGTUYEN_UPDATE(
    P_MAHOPDONG IN VARCHAR2,
    P_GIOITHIEU IN VARCHAR2,
    P_HOCVAN IN VARCHAR2,
    P_KINHNGHIEM IN VARCHAR2
...
END;
/
GRANT EXECUTE ON USP_HSUNGTUYEN_UPDATE TO RL_UNGVIEN;
