...
CREATE OR REPLACE PROCEDURE USP_GIAYTO_UPDATE(
    P_MAHOPDONG IN VARCHAR2,
    P_LOAI IN VARCHAR2,
    P_NOIDUNG IN VARCHAR2
)
...
BEGIN
    ...
    SET
        NOIDUNG = P_NOIDUNG
    WHERE MAUV = SYS_CONTEXT('USERENV', 'SESSION_USER')
    AND MAHOPDONG = P_MAHOPDONG
    AND LOAI = P_LOAI;
END;
/
GRANT EXECUTE ON USP_GIAYTO_UPDATE TO RL_UNGVIEN;


CREATE OR REPLACE PROCEDURE USP_GIAYTO_DELETE(
    P_MAHOPDONG IN VARCHAR2,
    P_LOAI IN VARCHAR2
)
AS
BEGIN
    DELETE FROM GIAYTOUV
    WHERE MAUV = SYS_CONTEXT('USERENV', 'SESSION_USER')
    AND MAHOPDONG = P_MAHOPDONG
    AND LOAI = P_LOAI;
END;
/
GRANT EXECUTE ON USP_GIAYTO_DELETE TO RL_UNGVIEN;
